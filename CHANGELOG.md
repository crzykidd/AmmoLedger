...
- **Restore directly from a backup already on the server.** New endpoint `POST /backup/restore/server` accepts `{ "filename": "<name from GET /backup/list>" }` and restores that file in place — no browser upload required. Admin only; filename is sanitized and confined to the backup directory by the same helpers used for download/delete; `.json` exports are rejected with a clear message (they go through the import preview/commit flow, not restore). The endpoint reuses the existing `.db` and `.zip` restore code paths verbatim, so it inherits the same WAL checkpoint, stats strip, integrity check, and durable atomic swap.
