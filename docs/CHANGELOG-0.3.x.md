# AmmoLedger Changelog — 0.3.x Archive

This file is a **frozen archive** of all changelog entries from v0.3.0
through v0.3.10. It is no longer updated.

For the active changelog from v0.4.0 onward, see [CHANGELOG.md](../CHANGELOG.md).
...
- **Choose your appearance.** A Light / Dark / Follow system mode picker in the Profile drawer lets you switch themes on demand; the choice persists per browser.
...
- **Pre-restore image snapshot — every restore now preserves the previous image directories.** Before placing restored content, both `firearm_photos/` and `products/` are renamed to `<name>.old` on the same filesystem. The previous `.old` snapshot (if any) is deleted first, so each restore replaces the prior safety copy. `.zip` restores then place the bundled contents; `.db` restores and JSON full-imports create empty directories (those formats carry no images, so the restored database would otherwise reference photos belonging to the previous install). The Backup page shows a persistent amber banner naming each snapshot directory, its file count, and total size; a Discard button (with confirmation) deletes the `.old` directories once the admin has verified the restore is correct. New admin endpoints: `GET /backup/restore-snapshots` and `POST /backup/restore-snapshots/discard`. The restore response now includes an `image_snapshots` payload describing which snapshots exist.
...
- **Restore directly from a backup already on the server.** New endpoint `POST /backup/restore/server` accepts `{ "filename": "<name from GET /backup/list>" }` and restores that file in place — no browser upload required. Admin only; filename is sanitized and confined to the backup directory by the same helpers used for download/delete. The endpoint reuses the existing `.db` and `.zip` restore code paths verbatim, so it inherits the same WAL checkpoint, stats strip, integrity check, and durable atomic swap. JSON exports already on disk are now also restorable in place via twin endpoints `POST /backup/import/preview/server` and `POST /backup/import/commit/server`, which route through the same `_import_preview_impl` / `_import_commit_impl` as the upload flow (pre-import safety backup, image rotation, forced logout all apply identically). The Backup History table on the Backup page now exposes both flows via a per-row Restore icon: `.db` / `.zip` rows open the existing destructive-restore confirm dialog; `.json` rows load the standard preview panel — no upload required. Closes #47
- **Restore and import now write structured audit log lines.** Every restore (`/backup/restore`, `/backup/restore/server`, deprecated `/backup/restore/sqlite`) and every JSON import (`/backup/import/commit`) emits a `Restore started` / `Import started` line on entry and a substantive completion line on success — restores report `restored database and N firearm photo(s)` (or `(no photos in backup)` for `.db` uploads), imports report `imported N record(s) across M table(s)`. The JSON import path previously logged nothing at all on success. Every line carries the triggering admin's username and the source filename.

### Changed

- **Restore and import failure / abort paths now log before raising.** Each impl wraps its existing failure branches with a uniform `Restore failed | actor=… | source=… | stage=<stage> | error=…` (or `Import failed | actor=… | source=… | error=…`) line so a rejected or crashed restore is no longer silent. The deliberate post-migration integrity-abort logs at `warning` (it's a rejected file, not a crash); everything else logs at `error`. Logging only — no behavior change.

### Fixed

- **Application log records from request handlers now reliably reach the container console — and each line appears exactly once.** Root cause proven by an in-worker probe at request time: inside the running uvicorn worker, `routers.backup.disabled == True` and the only root handler was a bare `StreamHandler(stderr)` at NOTSET — not the app's formatted stdout handler. So `logger.*` calls (including ERROR) were silently dropped by the disabled flag, and anything that did emit went unformatted to stderr; only `print(flush=True)` survived in `docker compose logs`. Cause: `main.py` imports all routers at module top, so each router's module-level `logger = get_logger("routers.<name>")` is created BEFORE uvicorn applies its own logging config — and the uvicorn `--reload` worker re-applies a config that sets `disable_existing_loggers: True` and strips our handler, AFTER our startup hook runs. A prior startup-time `basicConfig(force=True)` therefore did not survive. The fix lives in `backend/utils/logging.py`: `get_logger()` is now self-healing — every fetch (a) ensures root carries ONLY our single named, formatted stdout handler (foreign handlers, e.g. uvicorn's default, are removed so log lines do not double up — previously the same record was emitted once via uvicorn's bracketed `INFO  [routers.backup] ...` format and once via our `... | INFO | routers.backup | ...` format), and (b) clears `.disabled` and fixes `.propagate` on the returned logger. `configure_logging()` does the same plus a sweep over `logging.Logger.manager.loggerDict` re-enabling every existing logger and re-quieting `uvicorn.access` / `apscheduler` / `sqlalchemy.engine`. To repair state INSIDE the request (not just at startup), the hot-path restore/import handlers in `backend/routers/backup.py` (`_restore_sqlite_impl`, `_restore_zip_impl`, `import_commit`) now fetch their logger locally with `get_logger(__name__)` at call time. `LOG_LEVEL` is honored if set (else DEBUG in dev / INFO in prod).
- **Restore now performs a durable, same-filesystem atomic database swap.** The zip restore was extracting `ammoledger.db` into a `tempfile.mkdtemp()` directory on the container's overlay filesystem, then `shutil.move`'ing it onto the bind-mounted `/data` — a cross-filesystem copy + delete with no fsync. On Docker Desktop bind mounts on Windows (virtiofs → NTFS), the app reopened the file before all pages had flushed and `PRAGMA integrity_check` failed to even prepare, surfacing as `database disk image is malformed` after restore. The `.db`-only restore had the same shape, just less reliably fatal. Restore now stages a copy in the destination's *own* directory (guaranteed same filesystem), fsyncs the file, fsyncs the directory, runs `os.replace` for an atomic rename, and fsyncs the directory again before the engine reopens the database. The WAL checkpoint, stats strip, no-`ANALYZE` finalize policy, and `_assert_no_wal_sidecar` guard from the previous fix remain in place ahead of the swap.
- **Restore no longer corrupts the database with a dangling `sqlite_stat1` rootpage.** Root cause confirmed by raw-page inspection: restore ran `ANALYZE` on the migrated file, which allocated new pages for `sqlite_stat1` into the file's `-wal` sidecar; `shutil.move` then relocated only the main `.db`, leaving those committed-by-reference pages stranded in the WAL. The moved file's schema pointed at a rootpage past the end of the file, and the first schema-touching query after restore — login — failed with `malformed database schema (sqlite_stat1) - invalid rootpage`. Masked on ext4 by rename + fsync ordering, but reliably fatal on Docker Desktop bind mounts on Windows. Restore now checkpoints the WAL and drops out of WAL mode (`PRAGMA wal_checkpoint(TRUNCATE)` + `PRAGMA journal_mode=DELETE`) after Alembic upgrade and again after stripping the stats tables, so the restored file is a single self-contained file with no sidecar at move time. A belt-and-suspenders guard aborts the restore if a `-wal` somehow remains. Restore no longer runs `ANALYZE` — absent stats is the only state proven to boot clean; SQLite regenerates them lazily at runtime. Full-replace JSON imports apply the same checkpoint + strip policy on the live DB after commit. If the post-migration integrity check fails, restore is aborted and the live database is left untouched.
- **Startup auto-repair for databases already corrupted by a pre-fix restore.** On boot, the backend probes `sqlite_master` and, if it sees the dangling-rootpage error, drops the corrupt `sqlite_stat1` / `sqlite_stat4` schema rows via `PRAGMA writable_schema` and runs `VACUUM` to rebuild a clean file. Safe no-op on healthy databases. If auto-repair cannot complete, the user can still fall back to the manual `.recover` recipe in [docs/RECOVERY.md](docs/RECOVERY.md).
